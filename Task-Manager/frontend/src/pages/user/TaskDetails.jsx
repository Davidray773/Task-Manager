...
const TaskDetails = () => {
  ...
  const updateTodoChecklist = async (index) => {
    try {
      ...
      console.log("Error updating todo checklist:", error)
    } finally {
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 transition-all hover:shadow-md">
                ...
                <div className="mt-2">
                  ...
                  {task?.todoChecklist?.map((item, index) => {
                    ...
                    const isChecked = item?.completedBy?.some((userId) =>
...
const TodoCheckList = ({ text, isChecked, disabled, onChange }) => {
  return (
    <div className="flex items-center gap-3 p-3">
      <input
        ...
        disabled={disabled}
        onChange={onChange}
        className={`w-4 h-4 text-primary bg-gray-100 border border-gray-300 rounded outline-none cursor-pointer ${disabled ? "opacity-60 cursor-not-allowed" : ""}`}
      />
